...
class DiscordProcess:
    ...
    def script_path(self):
        if sys.platform == "win32":
            ...
            return os.path.join(
                base,
                "discord_desktop_core-%d" % version,
            ...
            config = os.path.expanduser(
                os.path.join(
                    os.getenv("XDG_CONFIG_HOME", "~/.config"), discord_version
                )
            )
            ...
            for subdirectory in os.listdir(config):
                ...
                try:
                    # versions are A.B.C
                    version_info = tuple(
                        int(x) for x in subdirectory.split(".")
                    )
    ...
    def preload_script(self):
        return os.path.join(
            self.script_path, "core", "app", "mainScreenPreload.js"
        )
    ...
    for proc in psutil.process_iter():
        try:
            ...
            if exe.startswith("Discord") and not exe.endswith("Helper"):
                ...
                if entry is None:
                    entry = executables[exe] = DiscordProcess(
                        path=path, exe=exe
                    )
...
def main():
    ...
    if b"contextBridge.exposeInMainWorld('BeautifulDiscord'," not in preload:
        preload = preload.replace(
            b"process.once('loaded', () => {",
            load_file_script.encode("utf-8"),
            1,
        )
    ...
    to_write = (
        entire_thing[:index]
        + css_reload_script.encode("utf-8")
        + entire_thing[index:]
    )
